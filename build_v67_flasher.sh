...
set -e
DIR="$HOME/Desktop/xiaozhi_flasher_v6_7"
mkdir -p "$DIR"
cd /tmp && rm -rf v6art && mkdir v6art && cd v6art
echo "[1/4] 下载CI产物..."
gh run download --repo fjsmchm/xiaozhi-esp32 --branch ci/ankong-v67 --name "$(gh run list --repo fjsmchm/xiaozhi-esp32 --branch ci/ankong-v67 --limit 1 --json displayTitle --jq '.[0].displayTitle' >/dev/null 2>&1; gh run list --repo fjsmchm/xiaozhi-esp32 --branch ci/ankong-v67 --limit 1 --json databaseId --jq '.[0].databaseId' | xargs -I{} gh api repos/fjsmchm/xiaozhi-esp32/actions/runs/{}/artifacts --jq '.artifacts[0].name')" . 2>/dev/null || gh run download --repo fjsmchm/xiaozhi-esp32 --branch ci/ankong-v67 .
BIN=$(find . -name "*ankong-v67*merged*.bin*" -o -name "merged.bin" | head -1)
