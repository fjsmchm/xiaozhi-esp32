...
set -e
DIR="/e/万银/项目/小智音箱/xiaozhi_flasher_v6_7e"
...
s=s.replace(m[1], b64);
s=s.replace('固件：小智AI v2.4.2 安控V6.6b（生产版）','固件：小智AI v2.4.2 安控V6.7e（试验3版）');
s=s.replace('本版升级：断线自动重连(5/10/30/60s退避) + 每分钟巡检 + 凌晨4点自愈重启；唤醒词「安控云」','本版升级：自研引擎+补齐SR模型/AEC配置(修坏启动与自激)；旧词安控云不识别');
if(s.indexOf('V6.7e（试验3版）')<0){console.error('徽标替换失败');process.exit(1)}
