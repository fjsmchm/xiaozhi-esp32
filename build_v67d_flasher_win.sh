...
set -e
DIR="/e/万银/项目/小智音箱/xiaozhi_flasher_v6_7d"
...
s=s.replace(m[1], b64);
s=s.replace('固件：小智AI v2.4.2 安控V6.6b（生产版）','固件：小智AI v2.4.2 安控V6.7d（试验2版）');
s=s.replace('本版升级：断线自动重连(5/10/30/60s退避) + 每分钟巡检 + 凌晨4点自愈重启；唤醒词「安控云」','本版升级：自研引擎阈值0.7+3秒不应期(修对话中误触发)；旧词安控云不识别');
if(s.indexOf('V6.7d（试验2版）')<0){console.error('徽标替换失败');process.exit(1)}
